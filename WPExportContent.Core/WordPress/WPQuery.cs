namespace WPExportContent.Core.WordPress
{
	public class WPQuery
	{
		...
		public string GetWPPosts
		{
			get
			{
				string result = $@"SELECT * FROM {_tablePrefix}posts WHERE post_status ='{Settings.Posts.PostStatus.PUBLISH}' AND post_type= '{Settings.Posts.PostType.POST}'";
		...
		public string GetWPProducts
		{
			get
			{
				string result = $@"SELECT * FROM {_tablePrefix}posts WHERE post_status ='{Settings.Posts.PostStatus.PUBLISH}' AND post_type= '{Settings.Posts.PostType.PRODUCT}'";
			...
			get
			{
				string result = $@"SELECT PM.* 
					FROM {_tablePrefix}postmeta AS PM 
						...
						ON P.ID = PM.post_id  
						WHERE P.post_status = '{Settings.Posts.PostStatus.PUBLISH}' 
